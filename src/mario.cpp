...
Mario::Mario(World* pWorld) : LevelObject(pWorld)
{
	...
	m_unstoppable_time = 2250;
	m_time = 0;
	m_coin_count = 0;
	rect = { 0.f, (float)Game::instance().get_window_size().y,(float)Game::instance().get_window_size().x, 20.f };
...
void Mario::update(float dt)
{
	if (!is_ejected()) {
		...
		if (m_state != MarioState::GROWING && m_state != MarioState::DECREASING) {
			...
			if (SDL_GetTicks64() > m_time + m_unstoppable_time) {
				m_invicible = false;
			}
			...
			if (!get_world()->is_won()) horizontal_movement(dt);
...
{
	if (m_invicible) {
		if (int((float)SDL_GetTicks64() / 100) % 2 != 0)
			return;
	}
...
void Mario::vertical_movement(float dt)
{
	...
	else if (get_pos().y + get_size().y >= Game::instance().get_window_size().y - 200.f) {
		Camera::instance().set_offset(Camera::instance().get_offset().x, -m_direction.y);
		set_y(Game::instance().get_window_size().y - 200.f - get_size().y);
		m_direction.x = 0.f;
			...
			if (level_object->is_drawable()) {
				if (level_object->get_collide_type() == LevelObject::COLLIDE_TYPE::NONE) {
					...
					if (SDL_IntersectFRect(&get_transf(), &level_object->get_transf(), &rect)) {
						...
						if (dynamic_cast<WinBar*>(level_object)) {
							get_world()->delete_gameobject(level_object);
							Mix_HaltChannel(0);
				...
				if (level_object->get_collide_type() != LevelObject::COLLIDE_TYPE::TOTAL) {
					...
					if (SDL_HasIntersectionF(&level_object->get_transf(), &get_transf())) {
						...
						if (dynamic_cast<Ennemy*>(level_object)) {
							if (dynamic_cast<Koopa*>(level_object)) {
								Koopa* koopa = dynamic_cast<Koopa*>(level_object);

								if (get_pos().y + get_size().y == koopa->get_pos().y + koopa->get_size().y) {
									kill();
								}
							}
							else if (dynamic_cast<MagicBullet*>(level_object)) {
								kill();
							}
...
void Mario::kill()
{
	if (m_state != MarioState::DECREASING && !m_invicible) {
		if (m_state == MarioState::SMALL) {
			...
			SoundManager::instance().play("shatter");
...
{
	if (fabs(m_direction.x) > 0.f || fabs(Camera::instance().get_offset().x) > 0.f) {
