					...
					if (SDL_IntersectFRect(&get_transf(), &level_object->get_transf(), &rect)) {
						...
						if (dynamic_cast<Mario*>(level_object)) {
							...
							if (tile->get_collide_type() == LevelObject::COLLIDE_TYPE::TOTAL) {
								if (get_pos().y + get_size().y >= tile->get_pos().y) {
									m_vel *= -1.f;
								}
							}
						}
							...
							if (dynamic_cast<Koopa*>(go->get())) {
								...
								if (!koopa->sliding() && abs(m_vel) > 0.f) {
									koopa->eject();
									HUD::instance().add_xp_text(koopa->get_pos().x, koopa->get_pos().y, 1000, 26, "stomp");
									break;
								}
							}
								...
								if (fabs(m_vel) == fabs(shell->get_vel())) {
									eject();
									HUD::instance().add_xp_text(get_pos().x - 20.f, get_pos().y, 400, 26, "stomp");

									shell->eject();
									HUD::instance().add_xp_text(shell->get_pos().x - 20.f, shell->get_pos().y, 400, 26, "stomp");
									...
									shell->eject();
									HUD::instance().add_xp_text(get_pos().x, get_pos().y, 400, 26, "stomp");
									break;
								}
							}
