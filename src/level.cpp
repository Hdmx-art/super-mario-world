		...
		for (int i = m_level_size.y - 1; i >= 0; i--) {
			...
			for (int j = 0; j < m_level_size.x; j++) {
				...
				if (number == 283)
					...
					add_basic_tile({ 170,85 }, j, i, tileset_pos, LevelObject::COLLIDE_TYPE::NONE);
				else if (number == 253)
					add_basic_tile({ 170,68 }, j, i, tileset_pos, LevelObject::COLLIDE_TYPE::NONE);
				else if (number == 233) 
					add_basic_tile({ 170,51 }, j, i, tileset_pos, LevelObject::COLLIDE_TYPE::NONE);
				else if (number == 213) {
					BasicTile* tile = add_basic_tile({ 170,34 }, j, i, tileset_pos, LevelObject::COLLIDE_TYPE::NONE);
					WinBar* bar = get_world()->add_gameobject_to_back<WinBar>();
					bar->set_pos(tile->get_pos().x + tile->get_size().x, tile->get_pos().y + tile->get_size().y);
					bar->set_init_pos(bar->get_pos().x, bar->get_pos().y + 350.f);
				}
				else if (number == 211)
					add_basic_tile({ 136,34 }, j, i, tileset_pos, LevelObject::COLLIDE_TYPE::NONE);
				else if (number == 162)
					...
					add_myster_block(j, i, tileset_pos, true);
				else if (number == 28)
					add_myster_block(j, i, tileset_pos, false);
				else if (number == 623)
					add_magikoopa(j, i, tileset_pos);
			}
		}
	}
...
void Level::handle_backgrounds(float dt)
{
	for (auto& background : m_backgrounds) {
		if (background->get_pos().x + (background->get_size().x - 1.f) < -(background->get_size().x - 1.f)) {
			background->set_x((background->get_size().x - 1.f) * 2.f);
		}
	}
}

BasicTile* Level::add_basic_tile(Vector2i rect, int j, int i, Vector2i tileset_pos, BasicTile::COLLIDE_TYPE collide)
{
...
void Level::add_myster_block(int j, int i, Vector2i tileset_pos, bool coin)
{
	MysterBlock* myster_block = get_world()->add_gameobject<MysterBlock>();

	if (coin)
		myster_block->set_item(MysterBlock::ItemType::COIN);
	else
		myster_block->set_item(MysterBlock::ItemType::MUSHROOM);
...
void Level::add_magikoopa(int j, int i, Vector2i tileset_pos)
{
	Magikoopa* magikoopa = get_world()->add_gameobject<Magikoopa>();

	magikoopa->set_pos(TILE_SIZE * j + tileset_pos.x * TILE_SIZE,
		Game::instance().get_window_size().y - 16 * TILE_SIZE + i * TILE_SIZE +
		tileset_pos.y * TILE_SIZE);
}
